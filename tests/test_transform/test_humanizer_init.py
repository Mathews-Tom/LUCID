"""Tests for the LUCIDTransformer orchestrator."""
...
class TestProtocolConformance:
    def test_satisfies_transformer_protocol(self) -> None:
        from lucid.core.protocols import Transformer

        hconfig, oconfig = _make_configs()
        detector = MagicMock()
        transformer = LUCIDTransformer(hconfig, oconfig, detector, profile="fast")
        assert isinstance(transformer, Transformer)

    def test_has_transform_method(self) -> None:
        hconfig, oconfig = _make_configs()
        detector = MagicMock()
        transformer = LUCIDTransformer(hconfig, oconfig, detector)
...
class TestSinglePass:
    def test_single_pass_returns_paraphrase_result(self) -> None:
        ...
        transformed_text = "The model does well on benchmarks."
        transformer = LUCIDTransformer(hconfig, oconfig, detector, profile="fast")

        transport = _mock_transport(transformed_text)
        import asyncio

        async def run_with_mock() -> TransformResult:
            ...
            try:
                result = await transformer._single_pass(chunk, detection, client)
                return result
            finally:
        ...
        assert result.transformed_text == transformed_text
        assert result.iteration_count == 1
        assert result.operator_used == "STANDARD"
    ...
    def test_single_pass_raises_on_missing_placeholder(self) -> None:
        ...
        transformer = LUCIDTransformer(hconfig_with_cite, oconfig, detector, profile="fast")
        ...
        async def run_with_mock() -> TransformResult:
            ...
            try:
                return await transformer._single_pass(chunk, detection, client)
            finally:
                await client._client.aclose()
...
class TestModelSelection:
    def test_fast_profile_uses_fast_model(self) -> None:
        ...
        detector = MagicMock()
        transformer = LUCIDTransformer(hconfig, oconfig, detector, profile="fast")
        assert transformer._model == "phi3:3.8b"
    ...
    def test_quality_profile_uses_quality_model(self) -> None:
        hconfig, oconfig = _make_configs()
        detector = MagicMock()
        transformer = LUCIDTransformer(hconfig, oconfig, detector, profile="quality")
        assert transformer._model == "llama3.2:8b"
    ...
    def test_quality_temperature(self) -> None:
        ...
        detector = MagicMock()
        transformer = LUCIDTransformer(hconfig, oconfig, detector, profile="quality")
        assert transformer._temperature == 0.5
